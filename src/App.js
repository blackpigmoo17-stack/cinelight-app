  ...
  <button
    onClick={() => {
      ...
      win.document.write(`
        ...
        <h2>${new Date().toLocaleDateString('th-TH', {year:'numeric',month:'long',day:'numeric'})}</h2>
        ${imgData ? `<img src="${imgData}" />` : ''}
        <div class="section">
          <div class="label">วิเคราะห์ฉาก</div>
          <div class="value">${analysis.scene_analysis || ''}</div>
        </div>
        <div class="section">
          <div class="label">แนะนำการจัดแสง</div>
          <div class="value">${analysis.lighting_recommendation || ''}</div>
        </div>
        ${(analysis.light_placement_detail || []).map((l, i) => `
          <div class="light">
            <div class="light-name">💡 ${l.light_name}</div>
            <div class="grid">
              ...
              ${l.hide_tip ? `<div class="item"><div class="label">ซ่อนไฟ</div><div class="value">${l.hide_tip}</div></div>` : ''}
            </div>
          </div>
        `).join('')}
